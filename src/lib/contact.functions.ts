...
export const contactSchema = z.object({
  ...
  projectType: z.enum(["Branding", "Website", "Social Media", "SEO", "Performance Marketing", "Full Marketing Campaign", "Other"]),
...
export const submitInquiry = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error } = await client.from("contact_inquiries").insert({
      ...
      project_type: data.projectType, budget_range: data.budgetRange, message: data.message,
    });
    if (error) throw new Error("We couldn't send your inquiry. Please try again.");
    return { ok: true };
  });